...
public class TaskWordAnnotationServlet extends BaseServlet {
	...
	protected void doSomething(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(skip!=null) {
			response.sendRedirect(page.getRandomPage("wordAnnotation.html"));
		}
		if(submit!=null) {
			try {
				var.WriteXMl(name.toString(), StandardTask.WORD_ANNOTATION, word, Arrays.asList(translation));
			} catch (Exception e) {
				response.sendRedirect("errorPage.html");return; //use return in order to skip line 49;
			}
			response.sendRedirect(page.getRandomPage("wordAnnotation.html"));
		}
	}
